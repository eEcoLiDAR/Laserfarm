...
do
  ...
  ssh -i $CLUSTERKEYPATH $USER@$BASESERVERNAME$s.$SERVEREXTENSION "/bin/bash --login -c \" nohup $RETILECODEPATH/run_retile_ahn3_vmj_inlist_robust.sh $s $IDNUM $NUMBERVMS $RETILECODEPATH $INPUTLIST $INPUTDIR $OUTPUTDIRECTORY $TEMPDIRECTORY "$EXTENT" $NUMBEROFTILES $NUMBEROFPROCESSES "$LP" > $RETILECODEPATH/rtahn3_VM$s.out & \"" 
