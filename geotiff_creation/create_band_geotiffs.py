...
"""
script to create geotiffs, one per band, from ply files with extracted features.
...
"""
adapted to geotiff expected orientation TODO: check that this has been done, i.e is arrayinfo[5] correct
"""
...
def terrainDataToGeoTiff(terrainData, terrainHeader, xres, yres, outputFileName):
    ...
    writeBandGeoTiffs(combinedTerrainFeatures, terrainHeader,geoTransform,outputFileName,ncols,nrows,bands)


def writeBandGeoTiffs(featureArrays, terrainHeader, geoTransform, outputFileName, ncols, nrows, bands):
    for i in range(bands):
        bandname = terrainHeader[3 +i]
        output_raster = gdal.GetDriverByName('GTiff').Create(outputFileName + "_" + bandname + ".tif", ncols, nrows, 1, gdal.GDT_Float32, ['COMPRESS=LZW'])
        #output_raster.SetMetadata(dict(zip(["band_{:02d}_key".format(i) for i in range(1, 1)], bandname)))
        output_raster.SetMetadata({"band_key":bandname})
